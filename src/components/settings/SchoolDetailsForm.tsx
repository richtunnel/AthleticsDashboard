...
export default function SchoolDetailsForm({ user }: Props) {
  ...
  const isDirty = useMemo(
    () =>
      ...
      form.schoolEmail.trim()    !== initialData.schoolEmail.trim()    ||
      form.schoolDistrict.trim() !== initialData.schoolDistrict.trim(),
    [form, initialData]
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const err = validate();
    if (err) { setAlert({ severity: "error", message: err }); return; }

    setLoading(true);
    setAlert(null);

    try {
      const res = await updateSchoolDetails({
        schoolName:     form.schoolName.trim(),
        teamName:       form.teamName.trim(),
        schoolAddress:  form.schoolAddress.trim(),
        schoolEmail:    form.schoolEmail.trim(),
        // Pass explicit district value (null = clear it, "" becomes null inside action)
        schoolDistrict: form.schoolDistrict.trim() || null,
      });
      ...
      <CardContent>
        ...
        <form onSubmit={handleSubmit}>
          <Stack sx={{ maxWidth: "768px" }} spacing={2}>
            <TextField
              size="small" label="School Name" name="schoolName"
              value={form.schoolName} onChange={handleChange}
              required fullWidth placeholder="e.g., Lincoln High School"
            />
            <TextField
              size="small" label="Mascot" name="teamName"
              value={form.teamName} onChange={handleChange}
              required fullWidth placeholder="e.g., Lincoln Lions"
            />
            <TextField
              size="small" label="School Email Address" name="schoolEmail"
              type="email" value={form.schoolEmail} onChange={handleChange}
              fullWidth placeholder="optional@school.edu"
            ...
            />
            <TextField
              size="small" label="School District" name="schoolDistrict"
              value={form.schoolDistrict} onChange={handleChange}
              fullWidth placeholder="e.g., Dallas Independent School District"
              helperText="Auto-detected from your school address. You can override this manually."
            />
