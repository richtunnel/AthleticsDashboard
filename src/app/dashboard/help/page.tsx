...
import {
  ...
  Chip, Button, Divider, Paper, Link as MuiLink,
} from "@mui/material";
    ...
    <Stack direction="row" gap={1.5} sx={{ mb: 1.25 }}>
      <Box
        sx={{
          ...
          fontSize: "0.68rem", fontWeight: 800, flexShrink: 0, mt: "1px",
        }}
      >
        {num}
      </Box>
  ...
  const sections: Section[] = [
      ...
      content: (
        <Box>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2, lineHeight: 1.75 }}>
            The most common cause of import errors is mismatched column names. Opletics maps your CSV
            headers to the right fields automatically — but the columns need to follow recognisable names.
          ...
          <Paper
            ...
            sx={{
              ...
              border: "1px dashed", borderColor: alpha("#6366f1", 0.4),
          ...
          <Step num={1}>Make sure your file is saved as <strong>.csv</strong> (not .xlsx or .numbers).</Step>
          <Step num={2}>Dates should be in <strong>YYYY-MM-DD</strong> format (e.g. 2025-09-05).</Step>
          <Step num={3}>Times should be in <strong>HH:MM</strong> 24-hour format (e.g. 15:00 for 3 PM).</Step>
          <Step num={4}>
            For <strong>Home/Away</strong>, use values like <em>Home</em>, <em>Away</em>, <em>H</em>, or <em>A</em> in your column.
            A column named <em>home_away</em>, <em>Location Type</em>, or <em>H/A</em> is automatically detected.
          </Step>
          <Step num={5}>Include <strong>gender</strong> in your sport column or as a separate column (e.g. "Boys Varsity Basketball")
            so Calendar View and Schedule Exchange display the correct league.</Step>
        </Box>
      ),
    },
    ...
    // ── 2. Calendar Sync ─────────────────────────────────────────────────────
    {
      ...
      content: (
        <Box>
          ...
          <Step num={2}>
            <strong>Look for the lock icon.</strong>{" "}
            In your Games table, find the sync icon at the right end of each row. A <em>lock icon</em> means the game is already synced and locked.
            If you see a plain sync icon (no lock), click it to push that game to your calendar.
          </Step>
          <Step num={3}>
            <strong>Verify your Google account.</strong>{" "}
            Make sure you are signed into the correct Gmail account — the one you connected inside Opletics.
            Syncing to a different Google account won't work until you reconnect with the right one.
          </Step>
          <Step num={4}>
            <strong>Check our status page.</strong>{" "}
            If sync is failing for all games, there may be a known incident.{" "}
    ...
    // ── 3. Seeing TBD ────────────────────────────────────────────────────────
    {
      ...
      content: (
        <Box>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2, lineHeight: 1.75 }}>
            TBD appears when Opletics can't match a column from your imported CSV to a recognised field.
            The <strong>Column Identity</strong> feature lets you manually map your column names.
          </Typography>
          <Step num={1}>
            Go to{" "}
          ...
          <Step num={2}>
            Select the column from your spreadsheet that holds the opponent / time / location data and map it to the correct field.
          </Step>
          <Step num={3}>
          ...
          <Step num={4}>
            If TBD persists after mapping, try re-importing your CSV with clearer column headers
            (e.g. rename <em>"opp"</em> → <em>"Opponent"</em> or <em>"tm"</em> → <em>"Time"</em>).
          </Step>
        </Box>
      ),
    },
    ...
    // ── 4. Emails not sending ────────────────────────────────────────────────
    {
      ...
      content: (
        <Box>
          ...
          <Step num={1}>
            <strong>Check your daily limit.</strong>{" "}
            Each plan has a daily sending cap. Go to{" "}
            <MuiLink component={Link} href="/dashboard/settings" underline="hover" sx={{ fontWeight: 600 }}>
              Settings → General
            </MuiLink>{" "}
          ...
          <Step num={2}>
            <strong>Verify recipient addresses.</strong>{" "}
            Remove any obviously invalid addresses from your email group (e.g. missing "@" or ".com"). Invalid addresses cause delivery failures for the whole batch.
          </Step>
          <Step num={3}>
            <strong>Narrow down your recipient group.</strong>{" "}
            If you're sending to a large group, try splitting it into smaller batches of 50–100 to isolate any problematic addresses.
          </Step>
          <Step num={4}>
            <strong>Check Email Logs.</strong>{" "}
            Go to{" "}
    ...
    // ── 5. Parent sync ───────────────────────────────────────────────────────
    {
      ...
      content: (
        <Box>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2, lineHeight: 1.75 }}>
            Parent calendar sync requires several pieces to be in place on both the AD and parent sides.
            Run through this checklist before escalating to support.
          </Typography>
          <Step num={1}>
            <strong>Delete and resend the request.</strong>{" "}
            In the{" "}
          ...
          <Step num={2}>
            <strong>Ask the parent to resync.</strong>{" "}
            Have the parent open their portal and tap <em>Resend Calendar Request</em> (or log out and back in).
            Sometimes a stale token is the only issue.
          </Step>
          <Step num={3}>
            <strong>Confirm your schedule is imported and your calendar is connected.</strong>{" "}
            Parent sync only works when you have games in Game Center <em>and</em> a Google Calendar connected on the{" "}
            <MuiLink component={Link} href="/dashboard/gsync" underline="hover" sx={{ fontWeight: 600 }}>
              Calendars
            </MuiLink>{" "}
            page.
          ...
          <Step num={4}>
            <strong>Check the parent's dashboard.</strong>{" "}
            The parent must have their own Google Calendar connected inside their parent portal dashboard.
            Without that, Opletics has nowhere to push the events.
          </Step>
        </Box>
      ),
    },
