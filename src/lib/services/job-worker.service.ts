    ...
    [JobType.EMAIL]: async (payload, job) => {
      ...
      if (recipients.length > 1) {
        ...
        if (result.success === 0 && result.failed > 0) {
          throw new Error(
            `[EMAIL] All ${result.failed} emails failed. First error: ${result.errors[0]?.error}`
          );
        }

        if (result.failed > 0) {
          console.warn(
            `[EMAIL] Partial failure: ${result.failed}/${recipients.length} emails failed. ` +
      ...
      if (!result.success) {
        throw new Error(
          `[EMAIL] Gateway send failed: ${result.error ?? "unknown error"}`
        );
      }
